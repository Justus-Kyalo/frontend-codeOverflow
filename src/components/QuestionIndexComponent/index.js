...
const QuestionIndex = () => {
  ...
  useEffect(() => {
    if (page === undefined || page === "undefined") setPage(1);
    dispatch(clearQuestions());
    dispatch(fetchQuestions(page, order, search, tag)).catch(() => {
      history.push("/NotFound");
    });
  }, [search, tag]);

  useEffect(() => {
    if (questions.length > 0) setTotalPages(questions[0].totalPages);
  ...
  const handleChangePage = (currentPage) => {
    ...
    dispatch(fetchQuestions(currentPage, order, search, tag)).catch(() => {
      history.push("/NotFound");
    });
  };
  ...
  const handleChangeOrder = (order) => {
    setOrder(order);
    setPage(1);
    dispatch(fetchQuestions(1, order, search, tag)).catch(() => {
      history.push("/NotFound");
    });
  };
      ...
      <div className="question-index-header">
        <div style={{ display: "flex", justifyContent: "space-between" }}>
          <div>
            ...
            {search !== undefined ? (
              <h4 style={{ marginTop: "5px", color: "gray" }}>
                Result for {search}
              </h4>
            ) : (
              <h4></h4>
          ...
          <button onClick={handleClick} className="question-index-button">
            Ask Question
          </button>
        </div>
        <div className="filter-buttons">
          {["Least Answered", "Most Answered", "Oldest", "Newest"].map(
            (opt) => (
              <button
                key={opt}
